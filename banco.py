...
class BancoClube:
    ...
    # PÁSSAROS
    # ================================================================
    def cadastrar_passaro(self, socio_id, nome, sigla_criador, numero_anilha, ano_anilha, cor="", anotacoes=""):
        if not nome:
        ...
        with self._conexao() as conn:
            ...
            cursor = conn.execute("""
                INSERT INTO passaros (
                    ...
                    codigo_ave, cor, anotacoes, criado_em
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                socio_id, nome, sigla_criador, numero_anilha, ano_anilha,
                codigo_ave, cor, anotacoes, datetime.datetime.now().isoformat()
            ))
            return cursor.lastrowid
    ...
    # ================================================================
    def editar_passaro(self, socio_id, passaro_id, nome, sigla_criador, numero_anilha, ano_anilha, cor, anotacoes):
        with self._conexao() as conn:
            ...
            conn.execute("""
                INSERT INTO passaros_edicoes (
                    ...
                    ano_anilha, cor, anotacoes, status, criado_em
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pendente', ?)
            """, (
                passaro_id, socio_id, nome, sigla_criador, numero_anilha,
                ano_anilha, cor, anotacoes, datetime.datetime.now().isoformat()
            ))
        ...
        with self._conexao() as conn:
            linhas = conn.execute("""
                SELECT 
                    ...
                    e.cor AS cor_nova,
                    e.anotacoes AS anotacoes_novas,
                    e.criado_em,
    ...
    def aprovar_edicao_passaro(self, edicao_id):
        with self._conexao() as conn:
            ...
            """, (
                ...
                edicao["ano_anilha"], edicao["cor"], edicao["anotacoes"],
                edicao["passaro_id"]
            ))
    ...
    def sortear_etapa(self, etapa_id):
        ...
        for pos, cpf in enumerate(ordem_final, 1):
            if cpf in por_dono and por_dono[cpf]:
